function out = mzxmlread2(filename,varargin)
...
% $Revision: 1.1.6.22 $   $Date: 2010/12/22 16:20:08 $
...
if isempty(regexp(str,'<\?xml','once'))
    error('bioinfo:mzxmlinfo:missingXMLdeclaration',...
        'File %s is missing an XML declaration.  %s might not be an XML file.',...
        filename,filename);
end

% Check if mzXML format
if isempty(regexp(str,'<mzXML|<msRun','once'))
    error('bioinfo:mzxmlinfo:notValidMZXMLFile',...
        'File %s is not an mzXML formatted file.',filename);
end
...
if isempty(parser)
    error('bioinfo:MZXMLREAD:FailedParser',...
            ['Problem creating StAX parser.',...
            ' Please check that %s is an mzXML formatted file'],filename);    
end
    ...
    if parser.isStartElement()
        ...
        switch elementName 
            ...
            case 'scan'
                ...
                if scanFlag
                    disp('Building scan substructure...'); %#ok<UNRCH>
                    scanFlag =0; %#ok<NASGU>
                end
            ...
            case 'peaks'
                ...
                if ~isempty(data)
                    switch out.scan(countScan+1).peaks.precision
                        ...
                        otherwise
                            warning('BIOINFO:mzxmlread:invalidPrecis','Invalid precision of %i found in scan, using single.', out.scan(countScan+1).peaks.precision);
                            % switch away from axis1
                            % out.(elementNameScan)(countScan+1).peaks.mz = processPeaks(b64.decode(char(parser.getElementText())),32);
                            out.(elementNameScan)(countScan+1).peaks.mz = processPeaks(b64.decode(data),32);
                    end
