function modification_editor_gui()
...
% table_data{:,8} = Quantitation Type Name      [text]
% table_data{:,9} = Characteristic Losses       [CSV string]
table_data = {};
...
uicontrol('Style', 'pushbutton', 'String', 'Done', 'Position', [650 10 75 25], 'Callback', @done);
    ...
    function setup_mods(~,~)       
        ...
        set(gcf,'name','Modification Setup','numbertitle','off', 'MenuBar', 'none');
    ...
    function change_mod(row)
        ...
        char_frag_handle = uicontrol('Style', 'edit',...
            'String', '',...
            'Parent', h3, 'Position', [500, 140, 200, 30]);
        ...
        if t_size(1) < row
            ...
            set(char_frag_handle, 'String', table_data{row,9});
            
            % Editing old row
            if isempty(table_data{row,7})
                % No MS1 or MS2 class selected
                set(MS1_handle, 'Enable', 'off');
                set(MS2_handle, 'Enable', 'off');
                set(u0,'Value',1);
                set(u1,'Value',0);
                set(u2,'Value',0);
            elseif strcmp(table_data{row,7}{1},'MS1')
                % MS1 class selected
                set(MS1_handle, 'Enable', 'on');                                
                set(MS2_handle, 'Enable', 'off');
                set(u0,'Value',0);
                set(u1,'Value',1);
                set(u2,'Value',0);
            elseif strcmp(table_data{row,7}{1},'MS2')
                % MS2 class selected
                set(MS1_handle, 'Enable', 'off');
                set(MS2_handle, 'Enable', 'on');
                set(u0,'Value',0);
        ...
        text(500,192.5, 'Characteristic Fragments (CSV)', 'Units', 'pixels', 'VerticalAlignment', 'middle');
        ...
        function mod_done(~,~)
            if ~isempty(get(handle_display_name, 'String')) && ~isempty(get(handle_AA, 'String'))                                                        
                ...
                table_data{row,9} = get(char_frag_handle, 'String');                
